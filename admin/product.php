<?php
    ...
    if(isset($_COOKIE['product_type'])) {
        $type = $_COOKIE['product_type'];
        setcookie("product_type", '', time() - 3600);
    } else {
        $type = 0;
    }

    if(isset($_COOKIE['product_status'])) {
        $status = $_COOKIE['product_status'];
        setcookie("product_status", '', time() - 3600);
    } else {
        $status = 2;
    }
    ...
    <div class="wrap">
        ...
        <div class="searchWrap">
            ...
            <div class="searchBox">
                <div class="box">
                    <span>分類:</span>
                    <select name="" id="type" class="searchorder">
                        <option value="0" <?php echo ($type == '0') ? "selected" : "";?>>全部</option>
                        <option value="1" <?php echo ($type == '1') ? "selected" : "";?>>床墊</option>
                        <option value="2" <?php echo ($type == '2') ? "selected" : "";?>>床架/床頭櫃</option>
                        <option value="3" <?php echo ($type == '3') ? "selected" : "";?>>衣櫃</option>
                    </select>
                    <span>狀態:</span>
                    <select name="" id="status" class="searchorder">
                        <option value="2" <?php echo ($status=='2') ? "selected" : "";?>>全部</option>
                        <option value="1" <?php echo ($status=='1') ? "selected" : "";?>>有效</option>
                        <option value="0" <?php echo ($status=='0') ? "selected" : "";?>>無效</option>
                    </select>
        ...
        <div class="listTable">
            <div class="listHead">好眠商品</div>
            <div class="listBodyWrap">
                <div class="addWrap">
                    <a href="product_add.php" class="addBtn">新增</a>
                    <select name="" id="page" class="pageSearch" style="margin-top:-15px">
                ...
                success:function(data) {
                    ...
                    if(data.status){
                        ...
                        for (const [key, val] of Object.entries(data.data)) {
                            ...
                            if(val.imgUrl) {
                                html +=     `<div class="img" style="position: relative;"><a href="../img/${val.imgUrl}" data-fancybox="b2"><img src="../img/${val.imgUrl}" alt="" style="max-width:70px; position: absolute; top: 0; right: 0; bottom: 0; left: 0; margin: auto; "></a></div>`;
                            } else {
                                html +=     `<div class="img"><img src="" alt="" style="max-width:70px"></div>`;
                            }
                            html +=     `<div class="name">${val.title}</div>`;
                            ...
                            html +=         `<a href="product_modify.php?id=${val.id}&type=${type}&status=${status}&page=${page}&number=${number}" class="edit"><i class="fas fa-edit"></i>編輯</a>`;
